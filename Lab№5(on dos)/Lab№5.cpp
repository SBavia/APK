...
void interrupt newI70h(...){ //генерация прерывания для часов
	milsec++;
	outp(0x70,0x0C);
	inp(0x71);
	outp(0x20,0x20);
...
void interrupt newI4Ah(...){  //генерация прерывания для будильника
	short value;
	oldI4Ah();  //стандартное пррывание
...
unsigned char get_param(int reg) //считать время
{
	unsigned char result;
	outp(0x70, reg);	
	result = inp(0x71);					//считать значение
	return result;
}

void wait(){  //ждем часы, если 7й бит == 1 => часы заняты
	do{
		outp(0x70, 0x0A);
	}while(inp(0x71) & 0x80 );
}

void disableUpdate(){  //отключить внутренний цикл обновления часов
    wait();
	outp(0x70, 0x0B);
...
void enableUpdate(){  //разрешить цикл обновления часов
	wait();
    outp(0x70, 0x0B);
...
void set_param(int value,int reg,int min, int max)
{
	...
	outp(0x70, reg);	
	outp(0x71, getBCDcode(value));					// запись значения( через BCD).
}
...
void getTotal()
{
	...
	// Считываем текущие время и дату и выводим их.
	disableUpdate();
	printf("The result of data and time:\n");
	hour = get_param(0x04);							// Час.
	minute = get_param(0x02);						// Минута.
	second = get_param(0x00);						// Секунда.
	year = get_param(0x09);							// Год.
	month = get_param(0x08);						// Месяц.
	day = get_param(0x07);							// День.
	weekday = get_param(0x06);						// День недели.
